    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            message = 'Cảm ơn quý khách <b>' + order.last_name + \
                ' ' + order.first_name + '</b> đã đặt hàng tại NaP Milk,'
            recepient = str(email_address)

            html_content = '''
            <p style="margin:4px 0;font-family:Arial, Helvetica, sans-serif;font-size:12px;color:#444;line-height:18px;font-weight:normal;">NaP Milk rất vui thông báo đơn hàng ''' + \
                str(order.id) + \
            ...
            for item in cart:
                ...
                <tr>
                    ...
                    str("{:0,.0f}".format(float(item['total_price']))) + \
                    '''</span></td></tr>'''
            ...
            <p style="margin:0;font-family:Arial, Helvetica, sans-serif;font-size:12px;color:#444;line-height:18px;font-weight:normal;">Trường hợp quý khách có những băn khoăn về đơn hàng, có thể liên hệ với chúng tôi theo số <b>(028) 12 345 678</b></p>'''
